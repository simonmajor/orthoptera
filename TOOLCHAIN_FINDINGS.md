# AI Toolchain Findings

This document records established knowledge derived from the Orthoptera AI-toolchain investigations.

It is separate from:

* `TOOLCHAIN_EXPERIMENTS.md`, which records the experimental history;
* `TOOLCHAIN_DECISIONS.md`, which records decisions about the toolchain;
* `TOOLCHAIN_WISHLIST.md`, which records useful capabilities that are not currently adopted.
...
Where a finding depends on an interpretation, that dependency should remain visible.
